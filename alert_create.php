...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($message)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO alerts (title, message, type, severity, is_active, expires_at, created_at) VALUES (?, ?, ?, ?, 1, DATE_ADD(NOW(), INTERVAL ? HOUR), NOW())");
            $stmt->execute([$title, $message, $type, $severity, $duration]);
            logActivity($pdo, $_SESSION['user_id'], 'Alert Created', "Broadcasted alert: $title");
